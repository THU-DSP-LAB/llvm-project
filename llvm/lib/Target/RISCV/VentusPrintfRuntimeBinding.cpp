//=== VentusPrintfRuntimeBinding.cpp - OpenCL printf implementation -------===//
//
...
#define DEBUG_TYPE "printfForVentus"
#define DWORD_ALIGN 4

namespace {
class VentusPrintfRuntimeBinding final : public ModulePass {

public:
  static char ID;

  explicit VentusPrintfRuntimeBinding();
...
class VentusPrintfRuntimeBindingImpl {
public:
  VentusPrintfRuntimeBindingImpl(
...
char VentusPrintfRuntimeBinding::ID = 0;

INITIALIZE_PASS_BEGIN(VentusPrintfRuntimeBinding,
                      "ventus-printf-runtime-binding", "Ventus Printf lowering",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_END(VentusPrintfRuntimeBinding, "ventus-printf-runtime-binding",
                    "Ventus Printf lowering", false, false)

char &llvm::VentusPrintfRuntimeBindingID = VentusPrintfRuntimeBinding::ID;

namespace llvm {
ModulePass *createVentusPrintfRuntimeBinding() {
  return new VentusPrintfRuntimeBinding();
}
} // namespace llvm

VentusPrintfRuntimeBinding::VentusPrintfRuntimeBinding() : ModulePass(ID) {
  initializeVentusPrintfRuntimeBindingPass(*PassRegistry::getPassRegistry());
}

void VentusPrintfRuntimeBindingImpl::getConversionSpecifiers(
...
bool VentusPrintfRuntimeBindingImpl::shouldPrintAsStr(char Specifier,
                                                      Type *OpType) const {
  if (Specifier != 's')
    return false;
  const PointerType *PT = dyn_cast<PointerType>(OpType);
  if (!PT || PT->getAddressSpace() != RISCVAS::CONSTANT_ADDRESS)
    return false;
...
bool VentusPrintfRuntimeBindingImpl::lowerPrintfForGpu(Module &M) {
  ...
  for (auto *CI : Printfs) {
    ...
    GlobalVariable *GVarOp = dyn_cast<GlobalVariable>(Op);

    if (ConstExpr || GVarOp) {
      GlobalVariable *GVar = nullptr;
      if (ConstExpr) {
        GVar = dyn_cast<GlobalVariable>(ConstExpr->getOperand(0));
      } else {
        GVar = GVarOp;
      }
